...
7. Without `X-Document-Type`, the chat model must select exactly one registered schema key, use the candidate's industry context to disambiguate similar document labels, return a bounded confidence score, and cite up to five distinct verbatim evidence excerpts while treating source text as untrusted. The API rejects ungrounded excerpts and caps reported confidence at 0.59, 0.74, 0.89, or 0.97 for zero, one, two, or at least three verified excerpts. Results at or above `CLASSIFICATION_AUTO_ACCEPT_THRESHOLD` are confirmed; results at or above `CLASSIFICATION_REVIEW_THRESHOLD` are retained and flagged for review; lower or invalid results remain unclassified. A valid header bypasses detection as a manual override.
...
| `DELETE` | `/v1/admin/documents` | Organization admin | Bulk-delete every active indexed document in the authenticated organization |
...
| `POST` | `/v1/admin/compute-sessions/release` | Tenant admin | Add held jobs to the open tenant session, creating one when needed |
| `POST` | `/v1/admin/compute-sessions` | Tenant admin | Open a manually bounded compute session |
| `POST` | `/v1/admin/compute-sessions/{id}/release` | Tenant admin | Append held jobs to a named open session for compatibility |
