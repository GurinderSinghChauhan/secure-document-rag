...
```text
Client -> TLS / mTLS gateway -> FastAPI API -> Ollama (embeddings + chat)
...
| Ollama | Self-hosted embedding and chat inference | Downloaded model weights |
...
6. Ollama creates dense embeddings using `EMBEDDING_MODEL`.
...
6. The chat prompt instructs Ollama to answer only from supplied context and disregard instructions found in documents.
...
| `OLLAMA_URL` | Private Ollama endpoint |
| `EMBEDDING_MODEL` / `CHAT_MODEL` | Models installed in the self-hosted Ollama service |
...
- Keep Qdrant, PostgreSQL, and Ollama off public networks. Do not expose their Docker ports.
