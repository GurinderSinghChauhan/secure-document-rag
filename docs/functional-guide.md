...
5. When there is no manual override, asks the self-hosted model to select one registered document type, using industry context to distinguish similarly named types, cite short verbatim supporting excerpts, and return a calibrated confidence score. The API verifies that every excerpt occurs in the document and caps confidence according to the number of distinct grounded excerpts. High-confidence results are confirmed, medium-confidence results are classified but flagged for review, and low-confidence or failed results remain unclassified.
...
The admin console's primary **Upload and index** action saves every selected file sequentially and adds all successfully uploaded jobs to the organization's open compute session. If no session is open, the API creates one automatically. New uploads join that same session while it is processing, and the worker drains the expanded document queue sequentially. The server derives and extends safety limits from the documents added; administrators do not enter or manage GPU minutes. Individual upload failures remain isolated, failed indexing jobs stay visible with a batch retry action, and completed jobs disappear from the live queue as polling updates arrive. The console loads held and failed work in 500-item pages, so recovery batches are not truncated.
...
Re-indexing is initiated by uploading the same file again, either while its existing document is active or after deleting it. The content fingerprint reconnects that upload to the existing document ID, reruns automatic classification and field extraction, and replaces its vectors and metadata. The service also retains the source payload on its completed ingestion job so an administrator can manually classify a document whose automatic classification failed and then continue the remaining extraction-to-indexing pipeline. Production database storage and backups must therefore be encrypted and governed as source-document data. The upstream system should remain the authoritative system of record.
...
An administrator can call `DELETE /v1/documents/{document_id}` for one document or `DELETE /v1/admin/documents` for every active document in the authenticated organization. Bulk deletion removes all matching vectors in one tenant-scoped Qdrant operation, soft-deletes the PostgreSQL metadata records together, and writes a metadata-only audit event containing the deleted count.

The admin console's **Indexed documents** section lists only active, searchable documents belonging to the administrator's organization. The list stays within a scrollable panel as it grows, and administrators can filter it instantly by filename, content type, role, or explicit user. An **All documents** / **Needs classification** toggle narrows the inventory to `unclassified` and failed automatic-classification records while preserving text search within that result. The inventory shows file size, chunk count, indexing date, access assignment, and classification status. Only documents with `unclassified` or `failed` automatic-classification status expose the manual type selector. Administrators select one or more of these documents, assign a manual type to every selected row, and choose **Apply types & extract data** once. The retained sources are parsed again and their schema-specific fields are extracted together through one bounded-compute session. Existing chunks, embeddings, and vectors remain unchanged. Confirmed and review-required documents do not expose this action. An administrator can also delete one entry or choose **Delete all documents** after an explicit confirmation; deleted entries disappear from the searchable inventory and can no longer contribute to answers. Bulk deletion applies to the whole organization, not only the current search. Held documents that have not completed processing remain in the separate compute queue.

Below 860px, the desktop sidebar is replaced by a horizontally scrollable mobile navigation bar. Dashboard, Insights navigation paths, Ask, Admin, and Platform Admin capabilities remain available according to the signed-in user's role; cards and dialogs stack to fit narrow screens.
