...
The dashboard summarizes only documents the signed-in person may query. It groups classified documents into field service, contract intelligence, litigation, healthcare, insurance, and accounts payable. Its independently scrollable inventory searches authorized documents by filename, document type, or industry without exposing other document summaries. Extracted metadata remains collapsed until requested, and users can inspect the configured document types and fields without seeing source content.
