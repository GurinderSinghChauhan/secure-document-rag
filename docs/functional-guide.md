...
The service retains extracted chunks in Qdrant, document metadata in PostgreSQL, and the source payload on its completed ingestion job so an administrator can re-index or correct classification without uploading the file again. Production database storage and backups must therefore be encrypted and governed as source-document data. The upstream system should remain the authoritative system of record. Documents without a retained completed-job source must be uploaded again before they can use the re-index action.
...
The admin console's **Indexed documents** section lists only active, searchable documents belonging to the administrator's organization. The list stays within a scrollable panel as it grows, and administrators can filter it instantly by filename, content type, role, or explicit user. It shows the file size, chunk count, indexing date, access assignment, and classification status. An administrator can choose a registered type and use **Classify & re-index** or **Reclassify & re-index** to correct a misclassification, or choose auto-detect to rerun automatic classification. Re-indexing rebuilds vectors and extracted metadata through the normal bounded-compute pipeline. An administrator can also delete one entry or choose **Delete all documents** after an explicit confirmation; deleted entries disappear from the searchable inventory and can no longer contribute to answers. Bulk deletion applies to the whole organization, not only the current search. Held documents that have not completed indexing remain in the separate compute queue.
