...
The admin console's primary **Upload and index** action saves every selected file sequentially, calculates the bounded GPU-minute ceiling returned by the server, and releases all successfully uploaded jobs into one compute session. Individual upload failures remain isolated and can be retried. If compute cannot start, the uploaded jobs stay safely held instead of being lost. Administrators can use the compute queue to select all other waiting files or clear the selection in one action. The console loads that queue in 500-item pages until every waiting document is available, so batches are not truncated at the first 500 jobs. Processing closes as soon as the selected batch finishes.
