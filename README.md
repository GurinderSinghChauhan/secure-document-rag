...
Production also requires HTTPS, verified email accounts, email-delivered invitations and password resets, a public-only `ALLOWED_HOSTS` list, and an explicit trusted-proxy list in `FORWARDED_ALLOW_IPS`. The service sends a restrictive Content Security Policy and browser security headers; terminate TLS at a trusted reverse proxy and preserve those headers. See [Product readiness](docs/product-readiness.md) before offering the system to customers.
